...
\begin{align}
&\rho_{pl}\approx\frac{\Sigma_p}{2a\sin{i}}\approx\frac{\sqrt{3}}{2}\frac{\Sigma_p\Omega}{v}
\end{align}
...
{\let\clearpage\relax\let\cleardoublepage\relax
\chapter{Evoluzione struttura planetaria: accrescimento e fase isolata.}\label{chap:gasaccretion}
}

Se il protopianeta raggiunge un valore critico di massa, dipendente debolmente da luminosit\'a generata da accrescimento dei pianetesimi e opacit\'a, per mantenere equilibrio idrostatico l'inviluppo gassoso del pianeta si contrae e si ha un afflusso di massa.
...
\begin{wrapfigure}[10]{lr}{0.5\textwidth}
\includegraphics[trim={0cm 0 0 0},clip, width=0.45\textwidth]{corotation}\label{fig:corotation}
...
\end{align}
Attornoall'orbita del pianeta si hanno orbite risonanti per elementi di fluido del gas. La condizione di risonanza \'e
\begin{align}
...
Le risonanze di Lindblad si hanno a:
\begin{equation}
...
Per tenere conto della pressione introduco l'altezza scala del disco $c_s=H\Omega$, quindi la posizione della risonanza \'e allontanata:
\begin{equation}
...
Per pianeti massicci il trasporto viscoso di momento angolare non \'e abbastanza efficiente quindi si forma un gap attorno al pianeta.
...
Il limite del regime lineare  \'e raggiunto quando il momento torcente dovuto al potenziale del pianeta \'e pari a quell odovuto alla viscosit\'a del disco:
\begin{equation}
...
Esistono condizioni per cui il momento torcente generato nella zona di corotazione pu\'o diventare dominante.
...
La regione di corotazione \'e definita da $\Omega(r)-\Omega_p=0$ cio\'e il potenziale del pianeta ruota alla stessa frequenza del fluido; utilizzando l'approssimazione impulsi nel regime di piccolo parametro d'impatto e bassa velocit\'a relativa si determina il valorelimite $x_s$ per cui se $x_s<b$ la particella di fluido compie un orbita a ferro di cavallo (\ref{fig:corotation}). Per pianeti di piccola massa $q<\num{e-4.5}$: $x_s\propto m_p^2$, mentre per pianeti massivi $q>\num{e-4}$: $x_s\propto R_H$.
