...
\end{equation}
e la dispersione di velocit\'a varia $\sigma\propto t\expy{1/4}$ fino a che non si raggiunge equilibrio con smorzamento da parte del disco di gas.
...
\end{equation}
La simulazione inizia quindi alla fine dell'accrescimento runaway con la determinazione della posizione iniziale degli embrioni planetari, di massa $\numrange{e-8}{e-2}\mearth$, scelta in maniera casuale: la probabilit\'a $p(a)\,da$ che l'embrione si sia formato in intervallo $da$ \'e inversamente proporzionale alla distanza orbitale $\Delta\propto a$ quindi la distribuzione \'e uniforme in $\log{a}$ tra \SIrange{0.05}{40}{\astronomicalunit}, verificato che il contenuto di solidi della sua fascia di accrescimento sia maggiore della sua massa:
\begin{equation}
